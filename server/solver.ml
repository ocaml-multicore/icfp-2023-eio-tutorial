...
type t = {
  ...
  mutable packages_cache : (Git_unix.Store.Hash.t * Packages.t Lwt.t) option;
}

type request = {
  pkg : OpamPackage.Name.t;
  commit : Git_unix.Store.Hash.t;
...
let packages t commit =
  ...
  | Some (c, p) when Git_unix.Store.Hash.equal c commit -> p
  | _ ->
...
let create store = { store; packages_cache = None }
